...
void Paladin::ability1(std::vector<std::unique_ptr<Monster>>& monsters, ParticleSystem& particles){
if(ability1Cooldown<=sf::Time::Zero){
    if(getMana()>=15.f){
        ...
        particles.addEmitter({hitbox.left,hitbox.top+25.f},10,{250,255},{250,255},{250,255});
        }
        else{attackRange=sf::FloatRect(hitbox.left,hitbox.top-100.f,100.f+hitbox.width, 200.f);
        particles.addEmitter({hitbox.left+hitbox.width,hitbox.top+25.f},10,{250,255},{250,255},{250,255});
        }
        for(int i=0; i<monsters.size(); i++){
            if(monsters[i]->hitbox.intersects(attackRange)){
                    monsters[i]->removeHealth(damage*0.5f+5);
                    particles.addTextEmitter(sf::Vector2f(monsters[i]->hitbox.left,monsters[i]->hitbox.top),Utils::toString(damage*0.5f+5,1),1,sf::Color::White,36);
...
void Paladin::ability2(std::vector<std::unique_ptr<Monster>>& monsters, ParticleSystem& particles){
if(ability2Cooldown<=sf::Time::Zero){
    if(getMana()>=20.f){
            ...
            particles.addEmitter(getCenter(),15,{200,250},{200,250},{0,50});
            for(int i=0; i<monsters.size(); i++){
                if(monsters[i]->hitbox.intersects(attackRange) && (monsters[i]->bodyType==INFERNAL || monsters[i]->bodyType==UNDEAD)){
                    ...
                    particles.addTextEmitter(sf::Vector2f(monsters[i]->hitbox.left,monsters[i]->hitbox.top),Utils::toString(damage*0.3f+3,1),1,sf::Color::White,36);
                }
            }
        addMana(-20.f);
...
void Paladin::ability3(std::vector<std::unique_ptr<Monster>>& monsters, ParticleSystem& particles){
    ...
    if(getMana()>=10.f){
        ...
        for(int i=0; i<monsters.size(); i++){
                ...
                if(monsters[i]->hitbox.intersects(attackRange) && (monsters[i]->bodyType==INFERNAL || monsters[i]->bodyType==UNDEAD)){
                    ...
                    particles.addTextEmitter(sf::Vector2f(monsters[i]->hitbox.left,monsters[i]->hitbox.top),Utils::toString(monsters[i]->getMaxHealth()*0.3f,1),1,sf::Color::White,36);
                    particles.addEmitter(lightning,1);
                }
            }
    addMana(-10.f);
